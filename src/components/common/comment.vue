    ...
    <div class="comments">
      <transition-group name='b-complete'>
        <div class="comment b-complete-item" v-for="(v, i) in comments" :key='v._id' :id='v._id'>
          <div class="figure"><img :data-dc='getFigure(v.user._id, v.user)' :alt="v.user.nickname || v.user._id" :src="v.user.avatar||'/static/assets/figure_blank_2.png'">
          </div>
          <div class="context">
...
export default {
  ...
  methods: {
    ...
    getFigure(_id, user) {
      ...
      let figures = {}
      figures = JSON.parse(localStorage.figurepool || '{}')
      // figures[_id] = { avatar: '/static/assets/figure_blank_2.png' }
      // localStorage.figurepool = JSON.stringify(figures)
      // console.info('figures', figures)
      // console.info('figure_id',figures[_id])
      // console.info(typeof figures[_id].createdTime)
      // console.assert(!figures[_id])

      if (!figures[_id] || typeof figures[_id].createdTime === 'undefined' || new Date() - figures[_id].createdTime > 2 * 60 * 1000) {
        api.userInfo(_id, info => {
          console.info(info)
          figures[_id] = {
          ...
          localStorage.figurepool = JSON.stringify(figures)
          // console.log(figures)
          user.avatar = figures[_id].avatar
          return 'ok'
        })
      }
      user.avatar = figures[_id].avatar
      return 'pending'
      // return { avatar: '/static/assets/figure_blank_2.png' }
    },
    ...mapActions([
      'newToast',
      'setPage'
